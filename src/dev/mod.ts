...
interface Manifest {
	...
	islands: { prefix: string; file: string }[]
}

export async function collect(directory: string): Promise<Manifest> {
	const routesDir = join(directory, './wb-vika-apis/routes')
	...
	try {
		...
		for await (const entry of routesFolder) {
			if (entry.isFile) {
				...
				routes.push({ prefix: routesDir, file })
			}
		}
	} catch (err) {
	...
	try {
		...
		for await (const entry of Deno.readDir(islandsDir)) {
			...
			if (entry.isFile) {
				...
				islands.push({ prefix: islandsDir, file })
			}
		}
	} catch (err) {
		if (err instanceof Deno.errors.NotFound) {
  ...
  routes: {
    ${routes
			.map(
				({ prefix, file }, i) =>
					`${JSON.stringify(
						`./${relative(dirname(manifestPath), prefix)}${file}`
					)}: $${i},`
			)
			.join('\n    ')}
  },
