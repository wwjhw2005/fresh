...
export class ServerContext {
	...
	static async fromManifest(
		...
		for (const [self, module] of Object.entries(manifest.routes)) {
			...
			if (!path.startsWith('/_') && !isMiddleware) {
				...
				const { handler, handlers } = module as RouteModule
				const setHandler = (
					newHandler: RouteModule['handler'],
					pattern: string
				) => {
					const cHandler = newHandler ?? {}
					if (
						component &&
						typeof cHandler === 'object' &&
						cHandler.GET === undefined
					) {
						cHandler.GET = (_req, { render }) => render()
					}
					const route: Route = {
						pattern,
						url,
		...
		try {
			...
			for await (const entry of entires) {
				...
				const contentType =
					typeByExtension(extname(path)) ?? 'application/octet-stream'
	...
	#handlers(): [
		...
		for (const { localUrl, path, size, contentType, etag } of this
			.#staticFiles) {
		...
		const unknownHandler: rutt.Handler<RouterState> = (req, ctx) =>
		...
		const errorHandler: rutt.ErrorHandler<RouterState> = (req, ctx, error) => {
			console.error(
...
function toPascalCase(text: string): string {
	return text.replace(/(^\w|-\w)/g, (substring) =>
